...
describe('Admin Api', function () {
  ...
  it('rejects unauthorized requests if authentication is false', async function () {
    const authedAdminApi = new AdminApi({ port: 3000 }, { app, middleware: createTokenAuthMiddleware(tokenService.introspect, tokenInfo => false), tokenService })
  ...
  it('allows authorized requests if authentication is true', async () => {
    const authedAdminApi = new AdminApi({ port: 3000 }, { app, middleware: createTokenAuthMiddleware(tokenService.introspect, tokenInfo => true), tokenService })
    await authedAdminApi.listen()
