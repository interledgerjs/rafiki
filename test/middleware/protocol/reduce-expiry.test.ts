...
import { ReduceExpiryMiddleware } from '../../../src/middleware/protocol/reduce-expiry'
...
describe('Reduce Expiry Middlware ', function () {
  let reduceExpiryMiddleware: ReduceExpiryMiddleware

  beforeEach(function () {
    this.clock = sinon.useFakeTimers(new Date(START_DATE).getTime())
    reduceExpiryMiddleware = new ReduceExpiryMiddleware({ minIncomingExpirationWindow: minExpirationWindow, minOutgoingExpirationWindow: minExpirationWindow, maxHoldWindow: maxHoldTime })
  })
  ...
  it('reduces the expiry time of incoming and outgoing packets by the configured time', async function () {
    ...
    const outgoingPipeline = setPipelineReader('outgoing', reduceExpiryMiddleware, async (packet: IlpPrepare) => {
      outgoingPacketDestinationExpiry = packet.expiresAt
      return mockFulfill
    })
    const incomingPipeline = setPipelineReader('incoming', reduceExpiryMiddleware, async (packet: IlpPrepare) => {
  ...
  it('throws InsufficientTimeoutError if source has already expired for outgoing packets', async function () {
    const outgoingPipeline = setPipelineReader('outgoing', reduceExpiryMiddleware, async (packet: IlpPrepare) => mockFulfill)
  ...
  it('throws InsufficientTimeoutError if source has already expired for incoming packets', async function () {
    const incomingPipeline = setPipelineReader('incoming', reduceExpiryMiddleware, async (packet: IlpPrepare) => mockFulfill)
