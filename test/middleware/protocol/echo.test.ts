...
import { EchoMiddleware } from '../../../src/middleware/protocol/echo'
...
describe('Echo middleware', async function () {
  ...
  const echoMiddleware = new EchoMiddleware({ getOwnAddress: () => 'test.connie.alice', minMessageWindow })
  const fulfillPacket = {
  ...
  describe('outgoing channel', function () {
    it('calls next for packets that arent addressed to me', async function () {
      ...
      const outgoingChannel = setPipelineReader('outgoing', echoMiddleware, next)
    ...
    it('creates type 1 echo packet and send to the incoming channel if flag = 0', async function () {
      const outgoingChannel = setPipelineReader('outgoing', echoMiddleware, async (packet: IlpPrepare): Promise<IlpReply> => fulfillPacket)
      const incomingChannel = setPipelineReader('incoming', echoMiddleware, async (packet: IlpPrepare): Promise<IlpReply> => fulfillPacket)
      const incomingChannelWriteSpy = sinon.spy(echoMiddleware.incoming, 'write')
