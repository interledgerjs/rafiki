      ...
      incoming: async ({ services, state: { ilp, peers } }, next) => {
        switch (ilp.req.destination) {
          case 'peer.route.control': {
            ...
            try {
              await services.connector.handleRouteControl(peers.incoming.id, deserializeCcpRouteControlRequest(ilp.rawReq))
          ...
          case 'peer.route.update': {
            ...
            try {
              await services.connector.handleRouteUpdate(peers.incoming.id, deserializeCcpRouteUpdateRequest(ilp.rawReq))
              ilp.rawRes = serializeCcpResponse()
            } catch (error) {
